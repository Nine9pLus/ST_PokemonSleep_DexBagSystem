...
namespace ST_PokemonSleep_DexBagSystem
{
	public partial class RegisterForm : Form
	{
		...
		private void btnRegister_Click(object sender, EventArgs e)
		{
			...
			{ MessageBox.Show("請輸入帳號與密碼"); return; }
			...
			{
				MessageBox.Show("此用戶已存在，請更換帳號或進行登入");
				return;
			}
			...
			// 執行註冊（資料庫新增）
			repo.AddUser(user);
			MessageBox.Show(@"註冊成功，請重新登入系統。\r\n您可以在「會員管理」頁面中修改暱稱與頭像。",
				"註冊完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
