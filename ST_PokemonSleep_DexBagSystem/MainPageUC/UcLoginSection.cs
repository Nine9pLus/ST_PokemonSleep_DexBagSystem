...
namespace ST_PokemonSleep_DexBagSystem
{
	public partial class UcLoginSection : UserControl
	{
		...
		private void btnLogin_Click(object sender, EventArgs e)
		{
			...
			#region (舊)用 AuthService 判斷帳密是否正確，並取得登入的 UserModel 物件。
			...
			if (userService.Login(account, password, out UserModel user))
			{
				// avatar已經內含在 user.AvatarFile 中
				LoginSuccess?.Invoke(this, new LoginEventArgs(user, user.AvatarFile));
			}
			else
			{
